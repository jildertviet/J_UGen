...
Memory::Memory(const std::string path, const std::size_t size,
               const bool persist)
    : _path(path), _size(size), _persist(persist){};
...
Memory::~Memory() {
  ...
  if (!_persist) {
    ...
    defined(_POSIX_VERSION) || defined(__ANDROID__)
...
inline Memory::Memory(const std::string path, const std::size_t size,
                      const bool persist)
    : _size(size), _persist(persist) {
  _path = "/" + path;
};
...
public:
  SharedMemoryReadStream(const std::string name, const std::size_t bufferSize,
   ...
   * @return float*
   */
  // TODO: might wanna use templated functions here like: <T>
  // readNumericArray()
...
public:
  SharedMemoryWriteStream(const std::string name, const std::size_t bufferSize,
                          const bool isPersistent)
      : _memory(name, bufferSize, isPersistent) {
  ...
  inline char getWriteFlags(const char type, const char currentFlags) {
    char flags = type;
  ...
  inline void destroy() { _memory.destroy(); }
